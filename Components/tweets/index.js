...
class App extends Component {
  renderError() {
    return (
      <Text className="error" style={styles.message}>
        {this.props.error}
      </Text>
    );
  }
  ...
  render() {
    return (
      <View style={styles.container}>
        <Search onSearch={this.props.searchForTweetsRequested} />
        <SearchTabs
        ...
        <View style={{width: '100%', alignItems: 'center', marginTop: 5}}>
          <TouchableOpacity
            ...
            onPress={async () => {
              const result = await AsyncStorage.getItem('TweetsNow');
              console.log(result);
            }}>
            <Text style={{textAlign: 'center'}}>Offline Tweets</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
